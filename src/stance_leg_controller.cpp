...
Eigen::VectorXd stance_leg_controller::get_action(void){
    ...
    foot_positions << l_position.x, r_position.x,
                      ...
                      l_position.z + hG, r_position.z + hG;
                    //   l_position.z, r_position.z;
    std::vector<double> force = Cmpc.ComputeContactForces(com_velocity,com_roll,com_droll,h_varphi,footcontact,
...
float alpha = 0.0001;
const std::vector<double> qp_weights {10.0,1.0,1.0,1.0,1.0};
...
std::vector<double> ConvexMpc::ComputeContactForces(
    ...
    state_ << com_roll,0,com_droll,com_velocity, h_varphi;
    for (int i = 0; i < planning_horizon; ++i) {
        ...
        desired_states_[i * kStateDim + 4] = h_varphi;
    }
    //calculate matrix
